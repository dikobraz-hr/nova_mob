    ...
    <div v-if="symbol" v-touch-swipe="onSwipe">
      <div class="fit row no-wrap justify-between items-start content-start">
        <q-btn
          ...
          size="lg"
        />
        <q-btn round class="q-mb-md" color="accent" text-color="dark" @click="goRandom" size="lg">
...
function loadSymbol() {
  ...
  const found = symbolsData.find((item) => item.id === id && item.category?.id === categoryId)
  if (found) {
    symbol.value = found
